...
class DataPresenter: NSObject {
    ...
    private var items: [MetaInfoDTO] = MetaItem.allCases.map { item in
        let info = MetaInfoDTO(item: item, dataString: "-")
        return info
    }
    ...
    private func getInfo(completion: ((Result<MetaInfo, ErrorResponse>) -> Void)?) {
